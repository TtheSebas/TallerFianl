      ...
      {paso === 4 && precioEstimado && (
        <div className="paso-4" style={{ animation: 'lightboxFadeIn 0.5s ease-out forwards', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            <button onClick={generarPDF} style={{ padding: '1rem', background: 'var(--espresso)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
              Descargar PDF RIMPE
            </button>
            <button onClick={() => setPaso(1)} style={{ padding: '1rem', background: 'transparent', color: 'var(--walnut)', border: '1px solid var(--sand)', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }}>
              Volver a cotizar
            </button>
          </div>
        </div>
